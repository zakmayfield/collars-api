...
export const resolvers = {
    ...
    Query: {
        ...
        // ::: Link :::
        linkFeed: async (parent, args, context) => {
            ...
            */
            ...
            if (linkFeed.length < 1) {
                return Promise.reject(new GraphQLError(`🚫 Nothing available, try adding some links.`));
            }
            return linkFeed;
        },
        ...
        linkComments: async (parent, args, context) => {
            const { linkId } = args;
    ...
    */
    ...
    Mutation: {
        ...
        deleteLink: async (parent, args, context) => {
            ...
            if (!linkToDelete) {
                return Promise.reject(new GraphQLError(`Link with ID: '${id}' does not exist.`));
            }
            if (linkToDelete.postedById !== user.id) {
                return Promise.reject(new GraphQLError(`User with ID: '${user.id}' did not create link with ID ${linkToDelete.id}`));
            }
            const deletedLink = await context.prisma.link
                ...
                if (err instanceof PrismaClientKnownRequestError &&
                    err.code === 'P2003') {
                    return Promise.reject(new GraphQLError(`Cannot delete non-existing link with id '${id}'.`));
                }
        ...
        updateLink: async (parent, args, context) => {
            ...
            if (linkToUpdate.postedById !== user.id) {
                return Promise.reject(new GraphQLError(`User with ID: '${user.id}' did not create link with ID ${linkToUpdate.id}`));
            }
        ...
        postCommentOnLink: async (parent, args, context) => {
            ...
            if (!linkToPostCommentOn)
                return Promise.reject(new GraphQLError(`Link with ID: '${linkId}' does not exist.`));
            if (!body)
            ...
            const newComment = await context.prisma.comment
                ...
                if (err instanceof PrismaClientKnownRequestError &&
                    err.code === 'P2003') {
                    return Promise.reject(new GraphQLError(`Cannot post comment on non-existing link with id '${linkId}'.`));
                }
                return Promise.reject(err);
            });
            return newComment;
        },
        deleteCommentOnLink: async (parent, args, context) => {
            const { user } = context;
            ...
            if (commentToDelete.postedById !== user.id) {
                return Promise.reject(new GraphQLError(`User with ID: '${user.id}' did not author comment with ID ${commentToDelete.id}`));
            }
            const deletedComment = await context.prisma.comment
                .delete({
                where: { id: commentId },
            })
                .catch((err) => {
                if (err instanceof PrismaClientKnownRequestError &&
                    err.code === 'P2003') {
                    return Promise.reject(new GraphQLError(`Cannot delete a non-existing comment with id '${commentId}'.`));
                }
                return Promise.reject(err);
            });
            return deletedComment;
        },
        updateCommentOnLink: async (parent, args, context) => {
            const { user } = context;
            ...
            if (commentToUpdate.postedById !== user.id) {
                return Promise.reject(new GraphQLError(`User with ID: '${user.id}' did not author comment with ID ${commentToUpdate.id}`));
            }
            const updatedComment = await context.prisma.comment
                .update({
                where: { id: commentId },
                data: {
                    body,
                ...
                if (err instanceof PrismaClientKnownRequestError &&
                    err.code === 'P2003') {
                    return Promise.reject(new GraphQLError(`Cannot post comment on non-existing link with id '${commentId}'.`));
                }
                return Promise.reject(err);
            });
